...
export function createNarukami(showcase=false,variant='base'){
 ...
 function palm(parent,isLeft){const p=new T.Group();parent.add(p);p.name=isLeft?'naru-left-hand':'naru-right-hand';box(p,0,-.02,.025,.125,.13,.07,m.glove);box(p,0,-.03,-.018,.115,.084,.012,m.panel);const fingers=[];for(let i=0;i<4;i++){const a=new T.Group(),b=new T.Group();a.position.set((i-1.5)*.032,.033,.02);p.add(a);box(a,0,.028,0,.028,.058,.033,m.glove);b.position.y=.05;a.add(b);box(b,0,.024,0,.025,.045,.028,m.skin);fingers.push({a,b});}const thumb=box(p,isLeft?.071:-.071,-.01,.048,.036,.078,.035,m.skin);thumb.rotation.z=isLeft?.5:-.5;box(p,0,-.115,.02,.138,.048,.095,m.glove);return {p,fingers};}
 ...
 function update({time=0,form='long',clip=null,seconds=0,attack=-1,heavy=false,side=0}={}){
  ...
  right.p.position.copy(new T.Vector3(0,-.135,.055).applyMatrix4(rig.matrix));right.p.rotation.set(-.35,-.22,p.angle-.55);
  left.p.position.fromArray(p.left);left.p.rotation.set(-.40,.10,-.68);
  right.fingers.forEach(({a,b},i)=>{a.rotation.set(-1.35*(1-p.open),0,(i-1.5)*p.open*.12);b.rotation.x=-1.18*(1-p.open);});
  left.fingers.forEach(({a,b},i)=>{a.rotation.set(i<2?-.18:-1.05,0,(i-1.5)*.10);b.rotation.x=i<2?-.08:-1.1;});
  const wrist=right.p.position.clone().add(new T.Vector3(.015,-.09,.035).applyEuler(right.p.rotation)),elbow=new T.Vector3(.87,-.83,.17);segment(fore,wrist,elbow);segment(sleeve,elbow,new T.Vector3(1.15,-1.09,.35));segment(lfore,left.p.position.clone().add(new T.Vector3(-.03,-.1,.02)),new T.Vector3(-1.14,-.92,.15));
