...
export function shotSpread(w,{aim=false,motion=0,airborne=false,crouched=false,recoil=0}={}){if(w.en==='MELEE')return 0;const moving=Math.max(0,(motion-.12)/.88);const base=w.en==='OPERATOR'?(aim?.0003:w.spread):w.spread*(aim?.55:1)*(crouched?.8:1);return base+moving*moving*(w.en==='OPERATOR'?.10:.052)+(airborne?.07:0)+recoil*.04;}
