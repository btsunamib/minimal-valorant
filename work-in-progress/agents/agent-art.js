...
export function createAgentModel(a,{team=0,armed=true}={}){
 ...
 function box(p,x,y,z,w,h,d,c,emit=0){const m=new T.Mesh(cube,material(c,emit));m.position.set(x,y,z);m.scale.set(w,h,d);m.castShadow=true;m.receiveShadow=true;p.add(m);return m;}
 ...
 const hitBody=box(model,0,.88,0,.43,1.46,.28,0);hitBody.material=new T.MeshBasicMaterial({visible:false});
