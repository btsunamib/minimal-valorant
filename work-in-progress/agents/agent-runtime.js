...
export class AgentRuntime{
 ...
 reset(e,points=e.kit?.points||0){const a=getAgent(e.agent);e.kit={points,charges:a.skills.map(s=>s.charges),cooldowns:[0,0,0,0],anchor:null,souls:0,ultimate:null};e.status={};e.agent=a.id;}
 clear(){for(const o of [...this.objects])this.remove(o);this.duel=null;for(const e of this.api.actors())e.status={};}
 remove(o){this.api.scene.remove(o.mesh);const i=this.api.wallMeshes.indexOf(o.mesh);if(i>=0)this.api.wallMeshes.splice(i,1);o.mesh.geometry?.dispose();o.mesh.material?.dispose();const j=this.objects.indexOf(o);if(j>=0)this.objects.splice(j,1);}
 ...
 cast(e,index,alt=false){
  ...
  if(!k||!s||this.api.phase()!=='live'||this.has(e,'suppress'))return false;
  ...
  let target;
  if(kind==='revive'){target=allies.filter(x=>!x.alive&&distance(x,e)<20).sort((x,y)=>distance(x,pos)-distance(y,pos))[0];if(!target){this.message(e,'附近没有可复活的队友');return false;}}
 ...
 tick(dt){this.t+=dt;
  ...
  for(const o of [...this.objects]){
   ...
   if(['bot','alarm','hound','drone','decoy','detain','seekers'].includes(o.kind)){
    const enemy=this.enemies(e).filter(x=>this.api.los(V(o.x,1,x.z===undefined?o.z:o.z),V(x.x,1,x.z))).sort((a,b)=>distance(a,o)-distance(b,o))[0];let dir=o.dir;
